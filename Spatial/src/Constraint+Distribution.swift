...
extension Constraint {
    ...
    * }
    */
   public static func distribute(horizontally views:[UIView], align:Alignment, spacing:CGFloat, offset:CGFloat = 0) -> [AnchorConstraint] {
      let xConstraints:[NSLayoutConstraint] = distribute(views, axis:.vertical, align:align, spacing:spacing, offset:offset)
    ...
    * IMPORTANT ⚠️️ Sets only anchors not sizes
    */
   public static func distribute(vertically views:[UIView], align:Alignment, spacing:CGFloat, offset:CGFloat = 0) -> [AnchorConstraint] {
      let xConstraints:[NSLayoutConstraint] = views.map{ view in
       ...
       let yConstraints = distribute(views, axis:.horizontal, align:align,spacing:spacing)
		 let anchors:[AnchorConstraint] = Array(zip(xConstraints,yConstraints))
		 return anchors
	}
   /**
  ...
  private static func distribute(fromStart views:[UIView], axis:Axis, offset:CGFloat = 0) -> [NSLayoutConstraint] {
		...
		for view in views {
         guard let toView:UIView = prevView ?? view.superview else {fatalError("view must have superview")}
			...
			if axis == .horizontal {
				anchors.append(Constraint.anchor(view,to:toView,align:.left,alignTo:.right,offset:offset))
			}else {//.vertical
				anchors.append(Constraint.anchor(view,to:toView,align:.top,alignTo:.bottom,offset:offset))
			}
			prevView = view
		}
		return anchors
  ...
  private static func distribute(fromEnd views:[UIView], axis:Axis, offset:CGFloat) -> [NSLayoutConstraint] {
	  ...
	  for view in views.reversed() {/*Move backwards*/
			...
			let offset = prevView == nil ? offset : 0
			if axis == .horizontal {
				anchors.append(Constraint.anchor(view,to:toView,align:.right,alignTo:.left,offset:offset))
			}else {//ver
				anchors.append(Constraint.anchor(view,to:toView,align:.bottom,alignTo:.top,offset:offset))
			}
         prevView = view
      }
		return anchors
